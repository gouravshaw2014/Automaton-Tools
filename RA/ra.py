        ...
        while current_states:
            ...
            for state, pos in current_states:
                ...
                for i in range(self.k):
                    if R[i] == d and (state, a, str(i)) in self.T:
                        matched = True
                        if(state,a,str(i)) in self.T:
                            for next_state in self.T[(state, a, str(i))]:
                ...
                if not matched and (state, a) in self.U:
                    j = self.U[(state, a)]
                    self.R[j] = d
                    if(state,a,str(j)) in self.T:
                        for next_state in self.T[(state, a, str(j))]:
                            next_states.add((next_state, pos + 1))
                    else:
                        continue
...
file_path = r"C:\Users\hp\OneDrive\Desktop\SAFA\ra1.txt"
